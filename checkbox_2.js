...
if (QL) {
    ...
    if (!fs.existsSync(`/${QL}/data/config/4399/config_2.yml`)) {
        console.log("您还没有填写cookies配置文件,请配置好再来运行8...\n配置文件路径/ql/data/config/4399/config_2.yml\n如没有文件复制一份config.yml.temple并改名为config_2.yml")
        return;
    } else{
    if(yaml.load) config = yaml.load(fs.readFileSync(`/${QL}/data/config/4399/config_2.yml`, 'utf8'))
